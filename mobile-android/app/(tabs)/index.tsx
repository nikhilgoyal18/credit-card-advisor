  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          data={nearbySearched ? results.filter(m => m.has_rewards) : results}
          ...
          ListFooterComponent={nearbySearched && results.some(m => !m.has_rewards) ? (
            <>
              <Text style={styles.sectionHeader}>Also nearby — estimated rewards</Text>
              {results.filter(m => !m.has_rewards).map(item => (
                <Pressable
                  key={item.id}
                  style={[styles.card, styles.cardDim]}
                  onPress={() => {
                    const cat = item.primary_category ?? item.category_estimate?.category;
                    router.push(`/recommend/${item.id}?name=${encodeURIComponent(item.canonical_name)}${cat ? `&category=${cat}` : ''}`);
                  }}
                >
                  <Text style={styles.merchantName}>{item.canonical_name}</Text>
                  <Text style={styles.category}>
                    {item.category_estimate
                      ? `~${item.category_estimate.best_rate}x est. · ${item.category_estimate.category.replace(/_/g, ' ').toLowerCase()} · ${item.category_estimate.card_name}`
...
const styles = StyleSheet.create({
  ...
  sectionHeader: { fontSize: 12, fontWeight: '600', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: 0.5, marginTop: 16, marginBottom: 8 },
